class UserController < Sinatra::Base
  ...
  #   log in the user using email/username and password
    post '/login' do
        ...
        if  user = Credential.find_by(username: user_data['username']) && user = Credential.find_by(password: user_data['password'])
          # user.to_json
          {username: user.username, user_id: user.user_id}.to_json
